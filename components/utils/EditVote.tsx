...
import { updateVoteService } from '@/services/room.service';
import { useParams } from 'next/navigation';
import { toast } from 'sonner';
import { useDispatch, useSelector } from 'react-redux';
import { RootState } from '@/redux/store';
import { updateVoteReducer } from '@/redux/slices/room.slice';
...
export default function EditVote({ onClose }: { onClose: () => void }) {
  const { vote } = useSelector((state: RootState) => state.userRoom);
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    if (parseRes.success && params.id) {
      if (isNaN(Number(parseRes.data.max))) {
        ...
        if (res.vote) {
          toast.success('Vote mis à jour avec succès');
