...
import { deleteVoteService } from '@/services/room.service';
import { useParams } from 'next/navigation';
import { useDispatch } from 'react-redux';
import { toast } from 'sonner';
import { deleteVoteReducer } from '@/redux/slices/room.slice';
...
export default function DeleteVote({
  ...
  const handleDelete = async () => {
    if (params.id) {
      setIsLoading(true);
      const res = await deleteVoteService({
      ...
      if (res.vote) {
        ...
        dispatch(deleteVoteReducer({ vote: res.vote }));
        onClose();
      }
    }
  };
  ...
  return (
    <div className="w-96 flex flex-col gap-6 p-6">
      ...
      <p className="text-[var(--text-primary-color)]">
        Êtes-vous sûr de vouloir supprimer ce vote ? Cette action est
        irréversible et supprimera toutes les données associées à ce vote.
      </p>
